...
with open(LAD, 'w') as fh:
    ...
    # ---- WALL LADDER VARIANTS, keyed to distance -------------------------
    # The masonry courses used to be one dark row in four at a FIXED screen
    # pitch, which is the defect an outside reviewer spotted immediately: the
    # bricks are the same size on a wall two cells away and one ten cells away.
    # Measured, a wall's top row (ktop) runs from 16 at two cells to 43 at ten,
    # so the wall's height on screen varies by 6x over the useful range while
    # the courses did not vary at all.
    #
    # Real scaling is texture mapping -- a multiply and a divide per pixel --
    # which is the entire frame-rate budget. The affordable approximation is
    # pre-scaled variants, exactly as the sprite renderer uses size bands: four
    # copies of the ladder, each with the course pitch its distance band wants,
    # selected through the dispatch table that is ALREADY indexed by ktop. The
    # selection therefore costs zero cycles; the only price is 1,158 bytes of
    # unrolled code, and the $0800 buffer-pairing constraint that used to make
    ...
    def emit_wall(fh, label, first, base):
        for k in range(first, VIEW_H // 2):
            if k < WBAND_TOP:
                ...
                # band; a dark cap reads as the wall falling out of the light.
                fh.write('        lda #$%02X\n' % wallcap(k))
            else:
